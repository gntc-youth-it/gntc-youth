...
export const TempleIntroSection = () => {
  ...
  const canEditActiveChurch = isMaster || ((user?.role === 'LEADER' || user?.role === 'MANAGER') && user?.churchId === effectiveTab)
    ...
    <section ref={sectionRef} id="temple-intro" className="py-24 bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Tabs value={effectiveTab} onValueChange={handleTabChange} className="w-full">
          ...
          {churches.map((church) => {
            const canEditThisChurch = isMaster || ((user?.role === 'LEADER' || user?.role === 'MANAGER') && user?.churchId === church.code)
            return (
              <TabsContent
                key={church.code}
                value={church.code}
